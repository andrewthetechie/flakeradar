...
function page(name: string): TestPage {
  const test: TestCase = {
    id: name.length, repo: "x/y", project: "default", fingerprint: name, suite: "",
    classname: "c", name, file: null, line: null, flakiness_score: 0.5, tier: "flaky",
    confirmed_flake_count: 0, last_status: "failed", last_seen_at: "2026-09-25T00:00:00Z",
    quarantined: false, quarantined_at: null, github_issue_number: null,
  };
...
describe("App", () => {
  it("ignores a slow response for a scope the user already left", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn((url: string) => {
      ...
      return json({
        total_tests: 1, flaky_tests: 1, suspect_tests: 0, confirmed_flaky_tests: 0,
        total_runs: 1, total_executions: 1, flake_threshold: 0.3,
      });
    }));
    ...
    await act(async () => { finishOld(new Response(JSON.stringify(page("oldtest")))); });
