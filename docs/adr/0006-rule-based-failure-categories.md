...
- The category is stored on each Execution, so a change to the rules applies only to new uploads. A maintenance command reclassifies the Executions that are still kept, and then rescores their Tests.
